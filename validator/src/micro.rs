    ...
    async fn next(
                ...
                if !in_current_state(&blockchain.head()) {
                    ...
                    if expected_next_ts <= now {
                        ...
                        let result = if cfg!(feature = "trusted_push") {
                            Blockchain::trusted_push(blockchain, Block::Micro(block.clone()))
                        } else {
                            Blockchain::push(blockchain, Block::Micro(block.clone()))
                        };
        ...
        let result = {
            ...
            if !in_current_state(&head) {
                ...
                // Use a trusted push since these blocks were generated by this validator
                let result = if cfg!(feature = "trusted_push") {
                    Blockchain::trusted_push(blockchain, Block::Micro(skip_block.clone()))
                } else {
                    Blockchain::push(blockchain, Block::Micro(skip_block.clone()))
                };
